...
| 证书有效期（起始-结束） |notBefore=May 22 12:16:05 2026 GMT notAfter=Aug 20 12:16:04 2026 GMT	 |
| 公钥算法 |	id-ecPublicKey	 |
| 签名算法 |	ecdsa-with-SHA384 |
| 证书链层级数 | 3|
| 根 CA 名称 |ISRG Root X1 |
| OpenSSL 验证结果（Verify return code） |0 (ok)	 |
| 是否为公开 CA / 第三方 CA 签发 |是（Let's Encrypt 是全球公开免费第三方CA）	
 |
...
| 项目 | 你的结果 |
| :--- | :------- |
| SHA-256 指纹（完整） |76:A6:6D:76:E0:EE:FA:30:8B:4F:AE:97:9B:B5:1D:12:72:59:DC:FA:68:9C:86:A5:86:F2:44:71:D7:E7:F2:A7 |
| SHA-1 指纹（完整） |30:BD:74:AD:BF:75:62:53:FF:75:EF:4E:9D:9D:9A:DF:24:9F:CB:CE |

### F. 真实 PKI 与私有 PKI 对比

| 对比项 | 真实公网 PKI（`${SITE}`） | 本实验私有 PKI |
| :--- | :--- | :--- |
| 最终信任根 |浏览器 / 操作系统内置的全球受信任根 CA 证书 |自行生成的私有根 CA 证书（My Root CA） |
| 浏览器默认是否信任 |是，默认已内置信任 |否，需要手动导入根 CA 证书后才会信任 |
| 证书签发者是否为第三方 CA |是，由具备公信力的第三方 CA 机构签发 |否，由自行搭建的私有 CA 签发，无第三方公信力 |
| 适合的使用场景 |面向公众的公网服务（如官网、电商平台等），需要全网通用信任|企业内部系统、内网服务、测试环境等仅需内部信任的场景 |
