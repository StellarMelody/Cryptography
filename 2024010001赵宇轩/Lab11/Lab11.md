...
| 签名公钥指数 e 的值 |65537（十六进制 0x10001） |65537（0x10001）
| 签名公钥模数 N 的前 16 位十六进制 |00:9f:31:85:87:1a:d7:bc |
...
| 项目 | 你的结果 |
| :--- | :------- |
| 原始消息内容（你写入的文字） |这是一条重要消息，需要签名保护。发送者：赵宇轩，学号：2024010001 |
| 密文文件 encrypted_message.bin 大小（字节） | 256|
| 密文 SHA-256 哈希值（完整的 64 位十六进制） |11163e41e54c1e10ceffaa1113c74455d6d55322c074f0ffd3b5f4f80232031d |
...
| 接收方应先验证签名还是先解密 | 先验证签名，验证通过后再执行解密|
...
> 答：数字签名提供身份认证、完整性保护、不可否认性三大安全保证：
身份认证
含义：能够确认数据的发送方确实持有对应签名私钥，排除第三方伪造身份。
实验体现：只有你拥有signature_private_key.pem，生成的签名仅能通过配套公钥验证；攻击者无私钥无法伪造合法签名，接收方可确认密文是你发送。
完整性保护
含义：保证数据在传输、存储过程中没有被篡改，任意一字节修改都会导致校验失败。
实验体现：向密文追加字符x得到篡改文件，使用原签名验证直接输出Verification failure；密文改动后 SHA256 哈希完全变化，公钥还原哈希与本地计算哈希不匹配，校验失败。
不可否认性
含义：签名者事后无法否认自己签署过该数据，第三方可独立举证归属。
实验体现：签名私钥唯一归属发送者，公钥完全公开，任何人都可复现验证过程；一旦签名验证通过，可证明密文由该私钥持有者生成，发送者无法抵赖发送行为。
...
> 答：1.运算效率限制
RSA 非对称运算速度极慢，直接对大容量密文逐块签名耗时极高；SHA-256 可极速将任意长度文件压缩为 32 字节固定哈希，仅对短小哈希签名，运算耗时仅毫秒级，适配工程场景。
2.RSA 输入长度上限约束
2048 位 RSA 单次仅能处理小于 256 字节的数据，长密文无法直接送入 RSA 签名运算；哈希统一输出固定长度摘要，完美适配 RSA 输入规范，无需复杂分块处理。
3.安全标准化约束
统一哈希输出长度让签名算法接口标准化；同时哈希的单向、抗碰撞特性弥补裸 RSA 签名的安全漏洞，杜绝消息替换、重放攻击。
...
> 答：2048 位 RSA 密钥模数 N 总长度为 2048 比特，换算字节：
2048÷8=256
 字节。RSA 签名运算输出长度等于模数的字节长度，因此signature.bin固定 256 字节。
4096 位 RSA 密钥模数为 4096 比特，
4096÷8=512
 字节，对应签名文件长度为 512 字节。
...
> 答：抗原像性（单向性）
定义：给定哈希值h，计算上无法找到任意消息m满足\(H(m)=h\)。
安全作用：攻击者仅拿到签名与哈希值，无法反向推导原始密文，保护传输数据隐私。
破坏后果：攻击者可通过哈希反向还原原始消息，泄露传输内容。


抗第二原像性
定义：给定消息\(m_1\)，无法找到另一条不同消息\(m_2\)，使得\(H(m_1)=H(m_2)\)。
安全作用：攻击者拿到合法签名后，无法构造替换密文并复用原有有效签名。
破坏后果：攻击者篡改密文后哈希不变，旧签名仍能验证通过，完整性保护完全失效。


抗碰撞性
定义：无法找到两条完全不同的消息\(m_1 \ne m_2\)，哈希值相等。
安全作用：杜绝攻击者预先制作两条哈希一致的文件，骗取签名后恶意替换。
破坏后果：攻击者准备正常文件与恶意文件，骗取正常文件签名后替换为恶意文件，验证依然通过，造成欺诈。
...
> 答：核心缺陷：2017 年研究者公开 SHA-1 碰撞攻击，可人为构造两份内容完全不同、但 SHA-1 哈希完全一致的文件，不满足抗碰撞性。
对数字签名的危害：签名绑定哈希值，两份文件哈希相同，一份文件的合法签名可直接用于另一份文件，签名绑定内容的特性失效。
攻击举例：攻击者制作文档 A（正常无风险合同）、文档 B（高额欺诈合同），两者 SHA1 哈希一致；让签名人对 A 签名，随后将文件替换为 B，接收方校验签名依然显示合法，完成欺诈。因此现代数字签名标准强制弃用 SHA-1，统一采用 SHA256、SHA3 系列哈希。
...
> 答：两组密钥分工
签名密钥对
签名私钥：对密文 SHA256 哈希执行 RSA 签名，仅发送方持有，严格保密；
签名公钥：全网公开分发，用于校验签名合法性，所有接收方均可使用。
加密密钥对
加密公钥：公开分发，用于加密明文，实现消息传输机密性；
加密私钥：仅接收方持有，用于解密密文，不可对外泄露。
攻击者获取公钥后的权限
可执行操作：加密消息、验证他人签名、导出密钥模数与公钥指数；
不可执行操作：生成合法签名、解密该公钥加密的数据、反向推导私钥 d。
...
> 答：核心安全目标不同
RSA 加密：解决机密性，防止第三方窃听，仅持有加密私钥的人可读原始内容；
数字签名：解决身份认证 + 完整性 + 不可否认性，任何人都可验证来源与篡改，不隐藏数据本身。
数学运算方向相反
加密流程：明文→公钥运算→密文（公钥加密、私钥解密）；
签名流程：数据哈希→私钥运算→签名（私钥签名、公钥验签）。
不能用加密替代签名的原因
加密仅能防止外人读取内容，无法证明发送者身份；攻击者可截获密文、解密篡改明文、重新加密发送，接收方无法识别密文被替换，不存在身份认证与不可否认性。
本实验流程「加密→哈希→签名→验证→解密」是互补设计：加密保机密，签名保来源与完整性，二者无法互相替代。
...
> 答：不可否认性定义：签名生成者无法事后否认自己签署过该数据，任何第三方都能独立验证签名归属，可作为可信举证依据。
数字签名具备、MAC 不具备该特性的核心原因：
数字签名基于非对称密码：签名私钥唯一归属签名人，公钥完全公开、独立可验证，私钥持有者无法抵赖；
MAC 消息认证码基于对称密钥：通信双方共用同一密钥，密钥双方均可生成合法 MAC，出现纠纷时无法区分是谁生成认证码，双方均可抵赖，无法律举证效力。
...
> 答：软件安装包发布
场景：官网提供软件安装包 + 配套数字签名；解决问题：防止黑客篡改安装包植入病毒，用户下载后校验签名，确认安装包来自官方开发者、传输未被篡改。
开源代码 Git 提交签名
场景：GitHub/GitLab 支持开发者用私钥对代码提交签名；解决问题：防止他人冒用账号提交恶意代码，仓库管理员可通过公钥验证提交者真实身份。
企业电子合同 / 电子发票
场景：企业用法人私钥签署电子合同、电子发票；解决问题：确认文件签署主体，防止合同篡改，同时提供不可否认性，满足法律存证要求。
...
> 答：数字签名本身无法证明签名时间：签名仅绑定文件内容与私钥，签名可在任意时间生成、复制，无法自带可信时间信息。
验证签名时间方案：使用可信时间戳服务（TSA）。
流程：签名完成后，将签名文件发送给权威第三方时间戳机构；机构用自身私钥对文件 + 标准世界时间共同签名生成时间戳；后续校验时同时验证文件签名与 TSA 时间戳，即可确认签名操作发生在时间戳标注的时刻。
...
> 答：2048 位 RSA（本实验选用）
安全性：短期（10 年内）安全，现有算力无法暴力分解模数；
性能：密钥生成、加密、签名运算速度适中，兼顾安全与效率，是当前通用标准。
1024 位 RSA
不安全：现有分布式算力可在数月内分解 1024 位模数，极易被破解，禁止用于签名、加密等安全场景。
4096 位 RSA
安全性：安全年限可达 20 年以上，抗量子计算短期威胁更强；
性能缺陷：模数翻倍，加解密、签名验证运算耗时提升 4~8 倍，占用更多存储与网络带宽；仅用于高安全等级场景（金融、政务根证书），普通场景无必要使用。
...
> 答：「Alice 签名文件，Bob 再对 Alice 的签名签名」具备实际意义，并非无意义操作。
适用场景：
证书信任链：根 CA 私钥给二级 CA 证书签名，二级 CA 再给用户证书签名，多层签名构建可信信任链；
多级审批流程：项目文件先由经办人 Alice 签名，部门主管 Bob 二次签名，证明文件经过两级人员确认，每层签名代表对应责任人认可内容；
多方审计存证：多机构依次对前一方签名再签名，完整留存所有参与方确认记录，便于事后追溯责任。
