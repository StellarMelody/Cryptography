...
| 公钥算法 |Public Key Algorithm: id-ecPublicKey（椭圆曲线 EC 公钥） |
...
| 是否为公开 CA / 第三方 CA 签发 |是（Let's Encrypt 公共免费 CA） |
...
| 项目 | 你的结果 |
| :--- | :------- |
| 证书主题（Subject） | C=CN, ST=Yunnan, L=Yuxi, O=My Root CA, OU=Certificate Authority, CN=My Root CAe|
...
| 项目 | 你的结果 |
| :--- | :------- |
| 证书主题（Subject） | C=AU, ST=Some-State, O=Internet Widgets Pty Ltd, CN=My Intermediate CA|
...
| SAN 中包含的域名数量 |4 个（www.myserver.com、myserver.com、mail.myserver.com、*.myserver.com） |
...
| 项目 | 你的结果 |
| :--- | :------- |
| SHA-256 指纹（完整） |1D:5B:B6:C9:AE:54:9D:3D:16:72:25:64:E3:9E:5E:D0:F9:F5:15:2D:36:9E:A6:F4:B4:42:84:83:72:3C:47:F9:15 |
| SHA-1 指纹（完整） | 2D:95:4E:D4:9A:99:B2:33:DB:E9:DF:29:5E:4F:DC:93:9D:CE:A8:2B|

### F. 真实 PKI 与私有 PKI 对比

| 对比项 | 	真实公网 PKI（3n7er.space） | 	本实验私有 PKI |
| :--- | :--- | :--- |
| 最终信任根 |	ISRG Root X1（系统预装公共根 CA） |自建 My Root CA（本地私有根 CA） |
| 浏览器默认是否信任 |	是（系统预装根证书） | 否（需手动导入根证书到信任库）|
| 证书签发者是否为第三方 CA |是（Let's Encrypt 第三方公共 CA） | 否（自建私有 CA 体系）|
| 适合的使用场景 |公网对外网站、互联网公开服务 | 企业内网、本地开发测试、内部系统|
...
> 答：数字证书本质是可信 CA 对「实体身份 + 实体公钥」的数字签名数据包，包含 X.509 规范定义的身份、公钥、有效期、扩展字段以及 CA 签名。
数字签名只能证明「消息未被篡改、签名者持有对应私钥」，但无法验证公钥本身归属；攻击者可同时替换公钥、签名、文件完成中间人劫持。数字证书引入可信第三方 CA 背书公钥，解决公钥可信性问题，弥补纯数字签名的安全缺陷。
...
> 答：自签名证书：实体用自身私钥对自己身份 + 公钥签名，Issuer 与 Subject 完全一致，无第三方担保。
浏览器警告原因：没有可信第三方 CA 背书，无法确认公钥归属，存在中间人劫持风险。
适用场景：内网开发测试、局域网内部服务、无公网暴露的私有系统，仅内部人员手动导入根证书信任。
...
> 答：根 CA 安全隔离：根 CA 私钥离线保管、极少使用，不直接签发终端证书，大幅降低根密钥泄露风险；
分层撤销隔离：若中间 CA 私钥泄露，仅需吊销该中间 CA，无需更换全网信任的根证书，成本极低；
权责分工：不同中间 CA 可划分业务、区域签发证书，统一管理；
路径长度限制：通过pathlen限制证书链层级，防止攻击者无限延伸伪造 CA 链条。
若根 CA 直接签发网站证书，根私钥需频繁联网签名，泄露后全网所有服务全部失效，维护成本极高。
...
> 答：根证书是自签名，无更高层级证书背书，依靠人工可信锚定：操作系统、浏览器厂商严格审核 CA 机构资质后，主动将其根证书预装在系统信任库。客户端默认信任列表内的所有根证书，列表外的自建根 CA 不会被默认信任。
...
> 答：pathlen:N 代表该 CA 证书最多向下延伸 N 层 CA 证书：
pathlen:0：当前 CA 只能签发终端实体证书，不能签发任何子 CA；
pathlen:1：当前 CA 可以签发 1 层中间 CA，中间 CA 不能再创建子 CA。
本实验中间 CA 设置pathlen:0，即使中间 CA 私钥泄露，攻击者也无法伪造二级 CA 批量签发虚假网站证书，收紧信任边界。
...
> 答：证书过期后浏览器 / 客户端直接拒绝连接，提示证书无效、不安全。
有效期设计目的：
强制定期轮换密钥，降低长期密钥泄露带来的风险窗口；
自动淘汰废弃域名、下线服务的证书，减少无效证书存量；
约束 CA 签发权限时效，过期证书签名不再具备法律效力。
Let's Encrypt 仅 90 天有效期：自动化 ACME 协议可自动续期，短周期能快速回收泄露 / 废弃证书，最小化中间人攻击窗口期。
...
> 答：证书私钥泄露、域名所有权变更、企业注销、证书误签发时，需要吊销证书。
CRL（证书吊销列表）：CA 定期生成完整吊销证书清单，客户端下载本地比对，文件体积大、更新延迟高；
OCSP（在线证书状态协议）：客户端实时向 CA 服务器单条查询证书状态，实时性强、流量小，现代 HTTPS 主流使用。
...
> 答：现代浏览器不再依赖 CN 字段匹配域名，仅校验 SAN 扩展，因此证书必须配置 SAN。
CN：仅传统通用名称字段，仅保留兼容作用，浏览器不再校验；
SAN：专门存储多域名列表，支持普通域名、通配符域名，是 TLS 域名匹配标准字段。
...
> 答：计算方式：将整张证书完整 DER 二进制编码做哈希（SHA1/SHA256），分段输出十六进制字符串。
证书钉扎（Certificate Pinning）：客户端首次连接服务器时保存证书指纹，后续每次连接强制比对指纹；即使证书由合法公共 CA 签发，只要指纹不匹配直接拒绝连接，抵御 CA 被入侵后签发虚假证书的中间人攻击。
...
> 答：*.example.com 仅匹配一级子域名：api.example.com、blog.example.com；
局限性：无法匹配多级域名a.b.example.com；任意子域名泄露私钥，所有子域名服务全部受影响，安全范围更大。
...
> 答：CT 是一套公开日志系统，所有公网 HTTPS 证书签发后必须录入公共日志。
解决问题：监控 CA 非法签发虚假域名证书，域名所有者、安全研究员可实时检索日志，快速发现 CA 违规签发行为，及时上报吊销、处置漏洞（如 DigiNotar 攻击事件）。
...
> 答：免费机制：依托互联网安全研究组织 ISRG 公益运营，由企业赞助，无人工审核成本；
ACME 域名验证：通过 HTTP-01（域名下放验证文件）、DNS-01（域名添加 TXT 记录）证明申请者拥有域名控制权；
短 90 天有效期：配套自动化续期工具，自动更新证书，缩短证书风险周期，避免长期未轮换密钥。
...
> 答：相同点
证书格式均为 X.509 v3，证书链层级结构一致（根 CA→中间 CA→终端证书），签名、有效期、密钥用途、SAN 等字段规范完全通用，验证流程逻辑相同。
不同点
信任根：公网根 CA 预装在系统信任库，私有根 CA 需手动导入；
签发主体：公网第三方合规 CA，私有自建 CA 无官方资质；
默认信任：浏览器原生信任公网证书，私有证书默认拦截告警；
使用场景：公网面向全网，私有仅限内网 / 测试环境。
浏览器信任 3n7er.space 是因为其证书链终点 ISRG Root X1 存在系统信任列表，自建服务器证书根 CA 不在列表内，因此默认不信任。
