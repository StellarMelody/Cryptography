...
Permalink: 任务一：观察真实网站证书链并追溯到本机根证书
步骤 1：在浏览器中查看证书
Permalink: 步骤 1：在浏览器中查看证书
...
Permalink: 步骤 2：使用 OpenSSL 获取证书
首先进入工作目录：
...
openssl s_client -connect ${SITE}:443 -servername ${SITE}
 \
  -showcerts 
< /dev/null > s_client_output.txt 2>/dev/null
Copy code to clipboard
...
Copy code to clipboard
注意这里用的是 2>&1 而不是 2>/dev/null。原因：depth=、subject=、issuer= 这些行是 OpenSSL 输出到 stderr 的，如果用 2>/dev/null就会把它们全部丢弃，awk 就什么都看不到。2>&1 把 stderr 合并进 stdout，awk 才能正常过滤。
输出示例（从下往上读就是从叶子到根）：
...
Issuer 中看到的
结论
Let's Encrypt、DigiCert、GlobalSign、Sectigo 等
是，填"是（机构名）"
自己填写的名字、企业内网名称
否
步骤 4：验证证书链
Permalink: 步骤 4：验证证书链
...
截图内容
文件名
浏览器证书详情页（含颁发者、主题、有效期、证书路径）
browser_cert.png
OpenSSL 链路摘要（depth / subject / issuer）+ 验证结果
openssl_cert.png
截图：
￼
￼
...
服务器证书
如何产生
openssl req -x509（直接自签名）
openssl req -new 生成 CSR，再由根 CA 用 openssl x509 -req 签发
同上，但由中间 CA 签发
谁来签名
自己签自己
根 CA 私钥签名
...
有（列出域名）
配置文件写法
root-ca.cnf（含 [dn] 和 [v3_ca]）
extensions.cnf（只有扩展字段）
san.cnf（只有扩展字段，含域名列表）
...
￼
任务二：创建私有自签名根 CA 证书
Permalink: 任务二：创建私有自签名根 CA 证书
步骤 1：生成根 CA 私钥
Permalink: 步骤 1：生成根 CA 私钥
...
截图内容
文件名
生成密钥、创建证书、openssl verify 验证成功的完整过程
root_ca.png
截图：
￼
￼
任务三：创建私有中间 CA 并由根 CA 签发
Permalink: 任务三：创建私有中间 CA 并由根 CA 签发
步骤 1：生成中间 CA 私钥
Permalink: 步骤 1：生成中间 CA 私钥
...
Permalink: 步骤 3：用根 CA 签发中间 CA 证书
命令 3-1：创建扩展配置文件
...
截图内容
文件名
创建 CSR、签发证书、验证成功的完整过程
intermediate_ca.png
截图：
￼
￼
根 CA 与中间 CA 证书对比
Permalink: 根 CA 与中间 CA 证书对比
做完任务二和任务三，手里有了两张 CA 证书，现在来仔细对比它们。先用命令把关键字段分别打出来：
echo "===== 根 CA =====" &&
 \
  openssl x509 -in 
~
/cryptography-lab12/root-ca/ca.cert.pem -noout \
    -subject -issuer -dates \
    -text 
2>/dev/null | grep -E "Subject:|Issuer:|Not Before|Not After|Public-Key|Basic Constraints|pathlen|Key Usage|CA:"

echo "===== 中间 CA =====" &&
 \
  openssl x509 -in 
~
/cryptography-lab12/intermediate-ca/intermediate.cert.pem -noout \
    -subject -issuer -dates \
    -text 
2>/dev/null | grep -E "Subject:|Issuer:|Not Before|Not After|Public-Key|Basic Constraints|pathlen|Key Usage|CA:"
Copy code to clipboard
...
Permalink: 差异一：Issuer 与 Subject
根 CA：
...
￼
任务四：签发私有服务器证书
Permalink: 任务四：签发私有服务器证书
步骤 1：生成服务器私钥
Permalink: 步骤 1：生成服务器私钥
...
步骤 2：创建服务器 CSR
Permalink: 步骤 2：创建服务器 CSR
命令 2-1：生成 CSR
openssl req -new -key server.key.pem -out server.csr \
  -subj 
"/C=CN/ST=Yunnan/L=Yuxi/O=My Company/OU=Web Services/CN=www.myserver.com"
Copy code to clipboard
参数与任务三步骤 2 相同，区别在于 Subject：
• CN=www.myserver.com：服务器证书的 CN 填域名，而不是名字。这是因为 TLS 验证时浏览器要对比 CN（或 SAN）与访问的域名是否匹配
步骤 3：创建 SAN 配置文件
Permalink: 步骤 3：创建 SAN 配置文件
命令 3-1：写入 SAN 配置
cat > san.cnf << 'EOF'
...
截图内容
文件名
创建 CSR、签发证书、验证完整证书链的过程
server_cert.png
截图：
￼
￼
服务器证书与 CA 证书对比
Permalink: 服务器证书与 CA 证书对比
...
Permalink: 差异一：basicConstraints 的演变
证书
basicConstraints
含义
根 CA
...
Permalink: 实验结果
A. 真实网站证书（任务一）
Permalink: A. 真实网站证书（任务一）
已在任务一结果表中填写，此处无需重复。
B. 私有根 CA 证书（任务二）
Permalink: B. 私有根 CA 证书（任务二）
项目
你的结果
...
￼
C. 私有中间 CA 证书（任务三）
Permalink: C. 私有中间 CA 证书（任务三）
项目
你的结果
证书主题（Subject）C=CN, ST=Yunnan, L=Yuxi, O=My Organization, OU=Intermediate CA, CN=My Intermediate CA
￼
证书颁发者（Issuer）C=CN, ST=Yunnan, L=Yuxi, O=My Root CA, OU=Certificate Authority, CN=My Root CA
￼
pathlen 值 0
...
￼
D. 私有服务器证书（任务四）
Permalink: D. 私有服务器证书（任务四）
项目
你的结果
证书主题（Subject）C=CN, ST=Yunnan, L=Yuxi, O=My Company, OU=Web Services, CN=www.myserver.com
￼
证书颁发者（Issuer）C=CN, ST=Yunnan, L=Yuxi, O=My Organization, OU=Intermediate CA, CN=My Intermediate CA
￼
CA 字段的值 CA:FALSE
￼
...
￼
E. 证书指纹（任务五）
Permalink: E. 证书指纹（任务五）
项目
你的结果
SHA-256 指纹（完整）
￼SHA256 Fingerprint=5C:2E:7D:91:0F:4B:33:A8:72:19:6E:CD:05:84:F0:27:BC:1A:E3:90:48:6F:5D:7C:2B:11:03:8E:AA:55:77:92
SHA-1 指纹（完整）
￼SHA1 Fingerprint=8D:3A:5F:10:7B:22:C9:44:E1:06:97:50:2F:DD:88:1E:BC:73:66:A1
F. 真实 PKI 与私有 PKI 对比
Permalink: F. 真实 PKI 与私有 PKI 对比
对比项
真实公网 PKI（${SITE}）
本实验私有 PKI
最终信任根
￼ISRG Root X1（全球公共根 CA）
￼My Root CA（本地自建自签名根 CA）
浏览器默认是否信任
￼是（系统预装根证书）
￼否（需手动导入根证书才会信任）
证书签发者是否为第三方 CA
￼是（Let's Encrypt 公开第三方 CA）
￼否（完全自建私有 CA 体系）
适合的使用场景
￼公网对外网站、面向全网用户的服务
￼企业内网、测试环境、内部业务系统、开发调试
￼
思考题
Permalink: 思考题
1. 数字证书的本质
...
答：数字证书本质是由可信 CA 签发、绑定实体公钥与身份信息的标准化电子文件，使用 CA 私钥签名。
解决的核心问题：公钥分发阶段的身份伪造中间人攻击，证明 “某个公钥确实属于对应实体”。
仅数字签名不足的原因：签名只能证明数据未篡改、签名者持有对应私钥，但无法确认签名者身份；攻击者可伪造公钥谎称是目标网站，证书引入第三方可信 CA 做身份背书，解决公钥归属信任问题。

￼
2. 自签名证书 vs CA 签名证书
Permalink: 2. 自签名证书 vs CA 签名证书
什么是自签名证书？为什么浏览器会警告自签名证书不安全？在什么场景下可以使用自签名证书？
答：自签名证书：证书签发者 (Issuer) 与使用者 (Subject) 完全相同，实体用自身私钥给自己签发证书，无第三方 CA 背书。
浏览器警告原因：系统信任库没有内置该自签名根证书，无法验证签名合法性，无法抵御中间人劫持风险。
适用场景：内网测试环境、开发调试、封闭企业内部系统，所有客户端可手动导入根证书建立信任；不适合公网对外服务。
￼
3. 证书链的必要性
Permalink: 3. 证书链的必要性
为什么需要证书链（根 CA → 中间 CA → 终端实体证书）？为什么不让根 CA 直接签发所有终端实体证书？
答：分层分担安全风险：根 CA 离线严格保管（最高安全等级），中间 CA 在线负责日常签发终端证书；若终端证书泄露，仅吊销中间 CA，无需泄露根私钥。
权限隔离：通过pathlen限制层级，根 CA 只允许签发中间 CA，中间 CA 仅能签发服务器终端证书，缩小攻击面。
管理轻量化：根 CA 数量极少，不用海量签发终端证书，分散签发压力，便于分级运维。
不让根 CA 直接签发终端证书：根私钥一旦联网频繁操作，泄露风险极大，一旦泄露整个信任体系全部崩塌。
￼
4. 根 CA 证书的信任
Permalink: 4. 根 CA 证书的信任
根 CA 证书是自签名的，为什么我们会信任它？操作系统和浏览器如何决定信任哪些根 CA？
答：我们信任自签名根 CA 不是密码学层面验证，而是操作系统 / 浏览器厂商人工审计后预置到本地信任列表，属于人为信任锚点。
系统 / 浏览器判定规则：内置一份经过安全审核的全球根 CA 清单；只有证书链终点根 CA 存在于该内置清单，整条证书链才判定可信；不在清单内的自签名根证书一律告警。

￼
5. pathlen 的作用
Permalink: 5. pathlen 的作用
中间 CA 证书中的 pathlen:0 是什么意思？如果 pathlen:1，会有什么区别？
答：pathlen:0：该 CA 只能签发终端实体证书，不能再签发任何下级 CA 证书，证书链到此终止。
若改为pathlen:1：允许当前 CA 再签发一层中间 CA，下级中间 CA 不可再创建 CA；证书链最多三层（根→本 CA→子 CA→服务器），层级限制放宽一层。
pathlen 用于控制证书链最大深度，防止无限递归创建多级 CA，限制攻击扩散范围。

￼
6. 证书过期
Permalink: 6. 证书过期
如果一个网站的证书过期了，会发生什么？为什么证书需要有效期限制？为什么 Let's Encrypt 的证书只有 90 天有效期？
答：网站证书过期后：浏览器直接抛出安全告警，HTTPS 连接阻断，用户无法正常访问网站。
有效期存在的原因：
降低密钥泄露风险：长期使用同一组公私钥会累积泄露概率，到期强制轮换密钥；
清理失效域名：域名所有权变更后，过期证书自动失效，防止旧证书滥用；
强制更新安全算法，淘汰老旧不安全加密套件。
Let’s Encrypt 仅 90 天：缩短有效期倒逼自动化续期，减少证书长期被盗用风险；配合 ACME 自动续期工具，人工维护成本极低，同时快速回收废弃域名证书。

￼
7. 证书吊销
Permalink: 7. 证书吊销
什么情况下需要吊销证书？CRL 和 OCSP 有什么区别？
答：吊销场景：服务器私钥泄露、域名所有权变更、企业业务下线、证书信息错误。
CRL 与 OCSP 区别：
CRL：证书吊销列表，CA 定期生成完整吊销清单，客户端下载全量列表本地校验；体积大、更新延迟；
OCSP：在线证书状态查询，客户端单独向 OCSP 服务器查询单张证书状态；实时、数据量小，依赖网络连通性。
￼
8. Subject Alternative Name
Permalink: 8. Subject Alternative Name
为什么现代证书通常包含 SAN 扩展？SAN 和 CN 有什么区别？
答：现代浏览器仅采信 SAN 字段校验域名，不再读取 CN，因此必须配置 SAN。
区别：
CN：旧标准公用名，仅支持填写单个域名，现代浏览器不再以此作为域名匹配依据；
SAN：扩展字段，支持同时填写多个域名、通配符域名，是 TLS 域名校验的标准字段。

￼
9. 证书指纹的用途
Permalink: 9. 证书指纹的用途
证书指纹是如何计算的？什么是证书钉扎（Certificate Pinning）？
答：计算方式：对证书完整 DER 二进制数据做哈希运算（SHA1/SHA256），输出十六进制指纹串，唯一标识单张证书。
证书钉扎 (Certificate Pinning)：客户端首次连接时缓存服务器证书指纹，后续每次连接强制比对指纹；即使攻击者拿到另一张合法 CA 签发的伪造证书，指纹不匹配会直接断开连接，抵御 CA 被入侵后的伪造证书攻击。

￼
10. 通配符证书
Permalink: 10. 通配符证书
通配符证书（如 *.example.com）可以匹配哪些域名？它有什么局限性？
答：*.example.com可匹配：api.example.com、mail.example.com、www.example.com等一级子域名。
局限性：
仅匹配一级子域名，无法匹配多级域名（a.b.example.com不匹配）；
若任意子域名私钥泄露，整张通配符证书全部失效；
无法区分不同子业务域名，权限粒度粗
￼
11. 证书透明度
Permalink: 11. 证书透明度
...
提示：查阅资料了解 CT 日志的作用。
答：CT（Certificate Transparency）是一套公开日志系统，所有公网 HTTPS 证书签发后必须录入公共 CT 日志，日志带密码学时间戳不可篡改。
解决问题：监控 CA 非法签发证书；一旦 CA 被入侵、违规签发仿冒域名证书，全网可在公开日志中发现，域名持有者能及时察觉并处置，杜绝隐蔽伪造证书监听流量。
￼
12. Let's Encrypt 的工作原理
Permalink: 12. Let's Encrypt 的工作原理
...
答：免费签发逻辑：非盈利机构，依托捐赠运营，使用 ACME 协议自动化签发，无人工审核成本；
ACME 域名验证：
HTTP-01：在网站根目录放置指定验证文件，CA 通过 80 端口访问校验所有权；
DNS-01：域名 DNS 添加 TXT 解析记录，校验域名控制权；
短期 90 天有效期：快速回收废弃证书，缩小密钥泄露攻击窗口；依靠自动化脚本自动续期，无需人工操作，弥补短期有效期带来的频繁更新成本；
以 3n7er.space 为例：域名持有者部署 ACME 客户端完成 DNS/HTTP 所有权校验，中间 CA E7 签发 90 天短期证书，证书链 E7→ISRG Root X1，浏览器信任内置根证书实现无告警 HTTPS 访问。
￼
13. 真实 PKI 与私有 PKI
Permalink: 13. 真实 PKI 与私有 PKI
为什么浏览器默认信任 3n7er.space 的 Let's Encrypt 证书，却不信任本实验签发的服务器证书？两者在证书格式、证书链结构和信任根上有哪些相同点和不同点？
答：信任差异原因
浏览器默认信任3n7er.space证书、不信任实验私有服务器证书的核心是信任根来源不同：
3n7er.space证书链根为ISRG Root X1，该根证书经过厂商安全审计，预装在 Windows、Chrome、Edge 等所有主流系统与浏览器内置信任列表；证书链可逐级验证到可信根，浏览器判定合法无告警。
实验私有服务器证书根是自建My Root CA自签名证书，操作系统、浏览器内置信任库不存在该根证书，没有预置信任锚点，无法完成完整可信校验，因此弹出不安全告警；只有手动导入自建根证书到系统信任库后，浏览器才会信任。
二、相同点
证书格式：均遵循 X.509 v3 标准 PEM 格式，包含版本、序列号、公钥、有效期、签发者、主体、V3 扩展字段（basicConstraints、keyUsage、SAN 等），解析规则完全一致。
证书链结构：都采用三级分层架构「根 CA → 中间 CA → 终端服务器证书」；根 CA 自签名，中间 CA 由根签发，服务器证书由中间 CA 签发，依靠多级签名逐级完成身份校验。
密码学逻辑：全部使用 RSA+SHA256 完成签名与身份验证，依赖非对称密码体系实现公钥身份绑定、防篡改。
